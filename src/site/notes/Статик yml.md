...
on:
  push:
    branches: ["main"]
    paths:
      - 'src/site/notes/**'  # Только при изменении заметок
      - '.github/workflows/**'
  workflow_dispatch:
...
concurrency:
  group: "pages"
  cancel-in-progress: true  # Новый деплой отменяет старый
...
jobs:
  build-and-deploy:
    ...
    steps:
      ...
      # 1. Скачиваем готовые шаблоны из obsidian-digital-garden
      - name: Download templates from obsidian-garden
        run: |
          echo "📥 Загружаю шаблоны..."
          mkdir -p templates
          
          # Основной CSS (можно использовать или адаптировать)
      ...
      # 2. Анализируем структуру как в obsidian-garden
      - name: Analyze notes structure
        id: scan
        run: |
          cd src/site/notes
          echo "🔍 Анализирую структуру заметок..."
          
          # Создаём структуру как в obsidian
          ...
          # Находим все markdown/html файлы
          first=true
          find . -name "*.md" -o -name "*.html" -o -name "*.htm" | while read file; do
            ...
            # Извлекаем заголовок (первая строка для .md)
            if [[ "$file" == *.md ]]; then
          ...
          echo "📊 Найдено заметок: $(grep -c '"file"' notes-structure.json)"

      # 3. Создаём главную страницу в стиле obsidian-garden
      - name: Create obsidian-style index
        run: |
          cd src/site/notes
          echo "🎨 Создаю страницу в стиле Obsidian Garden..."
          
          # Используем CSS из obsidian-garden
          cp ../../templates/style.css . 2>/dev/null || echo "Использую стандартный CSS"
...
<head>
    ...
    <style>
        /* Дополнительные стили из obsidian-garden */
        :root {
